...
e.g. code computing scene updates, can be performed (usually using CPU but it is fine to use GPU there as well).
...
* Access results stored in the output buffers only when they are frozen (``on_launch_finished``, ``on_rt_accum_done``) or use
  ``on_rt_accum_done`` to synchronize your code with the end of ray tracing; otherwise buffers might be still empty or contain
  undefined data.
